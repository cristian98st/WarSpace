...
int main(int argc, char *argv[])
{
    ...
    bzero(buffer, 1024);
    printf("Please identify yourselfbefore landing in the dock. (general, commander, wrrz)\n");
    fflush(stdout);
    scanf("%s", buffer); // cin role
    ...
    if (role != 2 && strcmp(buffer, "-1") == 0)
    {
        if (role == 1)
            cout << "Welcome, Commander!" << endl;
        else if (role == 0)
            cout << "Welcome, General!" << endl;
    }
    ...
    if (role == 1) // commander
    {
        ...
        {
            bzero(buffer, 1024);
            if ((read(sd, buffer, 1024)) < 0) // read the chosen role
            {
    ...
    else if (role == 2) // wrrz
    {
        if (strcmp(buffer, "0") == 0)
            ...
            printf("Ah, you have destroyed the most honorous of the human race, the General himself. They will make you the leader of the wrrz pack... this if none of your crew memebers will stab you in the back...\n Hey! Look behind!!... Oh, that's too bad... you have died...\n");
    }
